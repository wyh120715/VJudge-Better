...
(function() {
    ...
    const styles = `
        ...
        #contest-rank-table td {
            background: var(--vjb-cell-bg) !important;
            padding: 6px 4px !important;
            vertical-align: middle !important;
            text-align: center !important;
            font-size: 12px !important;
        ...
        #contest-rank-table td.prob.accepted span {
        ...
        #contest-rank-table td.prob:not(.accepted):has(span) {
            background: color-mix(in srgb, #e74c3c 15%, var(--vjb-cell-bg)) !important;
        }
        @media (prefers-color-scheme: dark) {
            #contest-rank-table td.prob:not(.accepted):has(span) {
                background: color-mix(in srgb, #e74c3c 20%, var(--vjb-cell-bg)) !important;
            }
        }
        ...
        #listStatus {
            ...
            table-layout: auto !important; /* 开启自适应窗格宽度，绝对不挤压溢出 */
        }
        #listStatus thead tr { background: transparent !important; }
        #listStatus th {
            font-weight: 700 !important;
            color: var(--vjb-text-main) !important;
            border: none !important;
            ...
            vertical-align: middle !important;
            text-align: center !important; /* 强制表头信息居中 */
        }
        #listStatus td {
            background: var(--vjb-cell-bg) !important;
            border: none !important;
            ...
            vertical-align: middle !important;
            color: var(--vjb-text-main) !important;
            text-align: center !important; /* 💡 强制全行数据水平垂直完美居中 */
        }
        #listStatus tbody tr:hover td {
            background: var(--vjb-hover-bg) !important;
        }

        /* 确保评测结果列和里面的大色块完美撑开、完全居中对齐 */
        #listStatus td.status, .status {
            text-align: center !important;
            vertical-align: middle !important;
        ...
        .vjb-status-block {
            ...
            border-radius: 4px !important; /* 排行榜微圆角大色块 */
            font-weight: 700 !important;
            font-size: 12px !important;
            white-space: nowrap !important; /* 绝不换行，自动向外优雅撑大窗格 */
            overflow: visible !important;
            text-decoration: none !important;
            line-height: 1.2 !important;
            transition: all 0.2s ease !important;
        }
        .vjb-status-block:hover {
        ...
        /* 规则矩阵颜色配置 */
        .vjb-color-green {
            background: color-mix(in srgb, rgba(46, 204, 113, 1) 22%, var(--vjb-cell-bg)) !important;
            color: rgba(46, 204, 113, 1) !important;
            border: 1px solid rgba(46, 204, 113, 0.2) !important;
        }
        .vjb-color-red {
            background: color-mix(in srgb, rgba(231, 76, 60, 1) 22%, var(--vjb-cell-bg)) !important;
            color: rgba(231, 76, 60, 1) !important;
            border: 1px solid rgba(231, 76, 60, 0.2) !important;
        }
        .vjb-color-orange {
            background: color-mix(in srgb, rgba(243, 156, 18, 1) 22%, var(--vjb-cell-bg)) !important;
            color: rgba(243, 156, 18, 1) !important;
            border: 1px solid rgba(243, 156, 18, 0.2) !important;
        }
        .vjb-color-purple {
            background: color-mix(in srgb, rgba(155, 89, 182, 1) 22%, var(--vjb-cell-bg)) !important;
            color: rgba(187, 120, 221, 1) !important;
            border: 1px solid rgba(155, 89, 182, 0.25) !important;
        }
        .vjb-color-bright-yellow {
            background: color-mix(in srgb, rgba(241, 196, 15, 1) 25%, var(--vjb-cell-bg)) !important;
            color: #ffeb3b !important;
            border: 1px solid rgba(241, 196, 15, 0.3) !important;
        }
        .vjb-color-dark-yellow {
            background: color-mix(in srgb, rgba(212, 172, 13, 1) 22%, var(--vjb-cell-bg)) !important;
            color: rgba(241, 196, 15, 0.95) !important;
            border: 1px solid rgba(212, 172, 13, 0.25) !important;
        }
        .vjb-color-grey {
            background: color-mix(in srgb, rgba(149, 165, 166, 1) 22%, var(--vjb-cell-bg)) !important;
            color: rgba(180, 195, 196, 1) !important;
    ...
    function processStatusTextElements() {
        ...
        elements.forEach(el => {
            // 1. 严格的环境防御隔离墙：绝对不允许碰表头、语言列、时间长度列等无关内容
            ...
            if (el.closest('#listStatus tbody td.status')) {
                ...
                // 面向代码详情页顶部的 summary 表格进行自适应列头对齐拦截
    ...
    function applySettings() {
        ...
        // 💡 模块化解耦解构：动态渲染并向 DOM 顶层插入独立的全局字体覆盖标签
    ...
    function createSettingsPanel() {
        ...
        panel.innerHTML = `
            ...
            <div class="vjb-group">
                ...
                <select id="vjb-set-font-code" class="vjb-select">
                    <option value="none" ${settings.fontCode === 'none' ? 'selected' : ''}>不替换</option>
            ...
            <div class="vjb-group">
                ...
                <select id="vjb-set-font-content" class="vjb-select">
                    <option value="none" ${settings.fontContent === 'none' ? 'selected' : ''}>不替换</option>
    ...
    function init() {
        ...
        // 建立高灵敏全息 DOM 文本监听沙盒：解决 AJAX 和页面加载时的异步重绘
